...
为 **Anima / Z-Image / Krea-2** 等新一代模型准备的一套效率节点。所有节点统一挂在 `⚡ WindMix` 根菜单下，方便查找，也避免与其他插件重名冲突。

## 核心功能

### 1. DiT效率加载器 —— 自动匹配 CLIP 与 VAE

覆盖 **Anima / Z-Image / Krea-2** 等新模型的加载，自动选择对应的 **CLIP 与 VAE**，不必再手工对照搭配。配套采样器与保存节点把「加载 → 采样 → 保存」收进同一条链路，少接一堆线，出图更快、更省显存。

### 2. XY 工作流 —— LoRA / diffusion_models 直观对比
...
> **配置要点**（两处都要设对，缺一不可）
> 1. `🎨 预设文本` 的**分隔符**填 `,\n---\n`
> 2. `🧩 Prompt Groups` 选择 **separator** 模式，**分隔符**填 `---`
>
> 设好后，预设里的每条提示词会被切成一个独立分组，依次出图。
