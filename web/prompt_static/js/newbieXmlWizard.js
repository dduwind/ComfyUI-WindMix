(function () {
    ...
    function bindEvents() {
        ...
        window.addEventListener("message", function (event) {
            if (!event.data || (event.data.handel !== "openPromptBox" && event.data.handel !== "responsePromptBox")) {
                return;
            }
            state.targetPromptType = event.data.nodeName === "PromptStudioNegativeOutput" ? "negative" : "positive";
            syncTargetWarning();
        });
    }
    ...
    function notifyPromptHost(positiveValue) {
        const randomId = localStorage.getItem("prompt_randomid");
        if (!randomId || !window.top || window.top === window) {
            return;
        }

        window.top.postMessage({
            handel: "changePromptBox",
    ...
    async function loadRemoteSettings() {
        try {
            const response = await fetch("/WM_prompt/physton_prompt/get_data?key=" + encodeURIComponent(SETTINGS_KEY));
    ...
    async function saveSettings() {
        saveLocalSettings();
        try {
            await fetch("/WM_prompt/physton_prompt/set_data", {
                method: "POST",
    ...
    function getPositiveInput() {
        return document.getElementById("prompt_text_input") || {
            value: "",
    ...
    function getNegativeInput() {
        return document.getElementById("prompt_text_neg_input") || {
            value: ""
        };
    }
})();
