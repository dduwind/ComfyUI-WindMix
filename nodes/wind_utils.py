...
loaded_objects = {
    ...
    "dit": [],  # (unet_name, clip_name, vae_name, model_family, shift, clip_type, model, clip, vae, [id])
}
...
def tensor_to_hash(tensor):
    """
    Compute a lightweight fingerprint of a tensor for cache key comparison.
    Instead of hashing the full tensor bytes (slow for large tensors), we use:
    - Shape and dtype (instant)
    - Sampled values from strategic positions (first, middle, last)
    - A checksum (sum of all values) as additional verification

    This is O(1) instead of O(n) while still providing excellent collision resistance
    for practical caching use cases.
    """
    # Hash shape and dtype (instant)
    shape_str = str(tuple(tensor.shape))
    dtype_str = str(tensor.dtype)

    # Sample a few values from strategic positions
    # Use .item() to get Python scalars, then convert to string for hashing
    try:
        flat = tensor.flatten()
        n = flat.numel()
        if n > 0:
            ...
            samples = [
                flat[0].item() if n > 0 else 0,
                flat[n // 2].item() if n > 1 else 0,
                flat[-1].item() if n > 0 else 0,
            ]
            # Compute checksum (sum of all values)
            checksum = float(tensor.sum().item())
        else:
            samples = []
            checksum = 0.0
    except Exception:
        # Fallback: if sampling fails, use a simpler approach
        samples = []
        checksum = 0.0

    # Combine all components into a hash
    fingerprint = f"{shape_str}|{dtype_str}|{samples}|{checksum}"
...
def load_lora(lora_params, ckpt_name, id, cache=None, ckpt_cache=None, cache_overwrite=False):
    ...
    def recursive_load_lora(lora_params, ckpt, clip, id, ckpt_cache, cache_overwrite, folder_paths):
        ...
        lora_name, strength_model, strength_clip = lora_params[0][:3]
