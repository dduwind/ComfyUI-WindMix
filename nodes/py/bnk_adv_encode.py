...
# ----------------------------------------------------------------------------
# Advanced weighting helpers (A1111 / compel / comfy++ / down_weight).
#
# These are ONLY used when weight_interpretation != "comfy". The default and the
# Efficient/DiT loader's hard-coded "comfy" path does NOT touch any of this — it
# delegates straight to ComfyUI's native CLIP.encode_from_tokens (see advanced_encode),
# which is byte-for-byte identical to the built-in "CLIP Text Encode (Prompt)" node.
# ----------------------------------------------------------------------------
...
def advanced_encode(clip, text, token_normalization, weight_interpretation, w_max=1.0, clip_balance=.5, apply_to_pooled=True):
    """
    Encode a prompt with the given CLIP.

    For the native ComfyUI "comfy" weighting (the default AND what the Efficient / DiT
    loaders hard-code) this delegates to ComfyUI's own scheduled encode path,
    `clip.encode_from_tokens_scheduled(...)`, which is byte-for-byte identical to the
    built-in "CLIP Text Encode (Prompt)" node. This preserves every key in the returned
    conditioning dict, including model-specific extras such as t5xxl_ids/t5xxl_weights
    for Anima / qwen_image multi-key CLIPs — exactly what prevents the "花图" bug.

    The A1111 / compel / comfy++ / down_weight modes only apply to a single, segmentable
    text-encoder key (SD1.5 'l', SDXL 'g'); multi-key DiTs fall back to the standard path
    because per-segment re-encoding is not meaningful across two separate encoders.
    """
    tokenized = clip.tokenize(text, return_word_ids=True)

    # SDXL family: keep the proven separate l/g path (already matches ComfyUI for "comfy").
    if isinstance(clip.cond_stage_model, (SDXLClipModel, SDXLRefinerClipModel, SDXLClipG)):
        embs_l = embs_g = pooled = None
    ...
    # Everything else (SD1.5, Flux, Anima/qwen_image multi-key, Llama, ...):
    # For the native "comfy" weighting, delegate to ComfyUI's standard scheduled encode path,
    # which is byte-for-byte identical to the built-in "CLIP Text Encode (Prompt)" node.
    # It returns the full conditioning dict (pooled_output + any model-specific keys such as
    # t5xxl_ids/t5xxl_weights for Anima), not just {"pooled_output": tensor}.
    if weight_interpretation == "comfy":
        scheduled = clip.encode_from_tokens_scheduled(tokenized)
        if scheduled:
            return scheduled[0][0], scheduled[0][1]
        return None, None

    # Explicit non-native weighting: only single-key CLIPs can be re-encoded per segment.
    ...
    if len(tokenized) > 1:
        ...
        if scheduled:
            return scheduled[0][0], scheduled[0][1]
        return None, None
    return advanced_encode_from_tokens(tokenized[key],
                                       token_normalization,
                                       weight_interpretation,
                                       lambda x: clip.encode_from_tokens({key: x}, return_pooled="unprojected"),
                                       w_max=w_max)
...
class AdvancedCLIPTextEncode:
    ...
    def encode(self, clip, text, token_normalization, weight_interpretation, affect_pooled='disable'):
        ...
        # `pooled` is either a full dict (comfy / multi-key scheduled path) or a pooled tensor.
        # Wrap tensor into the standard dict to stay compatible with downstream nodes.
        if isinstance(pooled, dict):
            return ([[embeddings_final, pooled]],)
